...
Map<String,dynamic> availabletoJson()
{
 ...
 DateTime extractDate(String date)
 {
   ...
   return DateTime(int.parse(splitedDate[0]),int.parse(splitedDate[1]),int.parse(splitedDate[2]));
 }
  TimeOfDay extractTime(String date)
 {
   final splitedDate=date.split(':');
   return TimeOfDay(hour:int.parse(splitedDate[0]),minute:  int.parse(splitedDate[1]));
 }
}